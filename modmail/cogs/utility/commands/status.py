...
from modmail.backends.common import Activity, ActivityType, StatusType
...
@wrap(app_commands.describe, status=_("cmd-status-param-status-description"))
@lazy_hybrid_group(
    name=_("cmd-status-name"), fallback=_("cmd-status-fallback-name"), description=_("cmd-status-description")
)
async def status_command(self: Utility, ctx: commands.Context[Bot], *, status: str | None = None) -> None:
    ...
    if not status:  # No status provided, show the current status
        ...
        if current_status is not None and current_activity is not None:
            current_status_message = await self.translate(
                ctx, _("msg-status-current-status", status=current_status)
            )
            current_activity_message = await self.translate(
                ctx, _("msg-status-current-activity", activity=current_activity)
            )

            await self.reply(ctx, f"{current_status_message}\n{current_activity_message}", ephemeral=True)
        elif current_status is not None:
            await self.reply(ctx, _("msg-status-current-status", status=current_status), ephemeral=True)
        elif current_activity is not None:
            await self.reply(ctx, _("msg-status-current-activity", activity=current_activity), ephemeral=True)
        else:
            await self.reply(ctx, _("msg-status-no-status"), ephemeral=True)
        return

    status_name_mapping: dict[str, StatusType] = {
        (await self.translate(ctx, _("model-status-online-name"))).casefold(): StatusType.online,
        (await self.translate(ctx, _("model-status-idle-name"))).casefold(): StatusType.idle,
        (await self.translate(ctx, _("model-status-dnd-name"))).casefold(): StatusType.dnd,
        (await self.translate(ctx, _("model-status-dnd-full-name"))).casefold(): StatusType.dnd,
        (await self.translate(ctx, _("model-status-offline-name"))).casefold(): StatusType.offline,
        (await self.translate(ctx, _("model-status-invisible-name"))).casefold(): StatusType.offline,
    }
    ...
    if status.casefold() in status_name_mapping.keys():
        ...
        await self.reply(ctx, _("msg-status-set-status", status=status_type), ephemeral=True)
        return

    activity_name_mapping: dict[str, ActivityType] = {
        (await self.translate(ctx, _("model-activity-playing-name"))).casefold() + " ": ActivityType.playing,
        (await self.translate(ctx, _("model-activity-streaming-name"))).casefold() + " ": ActivityType.streaming,
        (await self.translate(ctx, _("model-activity-listening-name"))).casefold() + " ": ActivityType.listening,
        (await self.translate(ctx, _("model-activity-watching-name"))).casefold() + " ": ActivityType.watching,
        (await self.translate(ctx, _("model-activity-competing-name"))).casefold() + " ": ActivityType.competing,
    }
    ...
    for activity_name, activity_type in activity_name_mapping.items():
        if status.casefold().startswith(activity_name) and len(status) > len(activity_name):
            ...
            await self.reply(ctx, _("msg-status-set-activity", activity=activity), ephemeral=True)
            return
...
@wrap(commands.guild_only)
@status_command.command(name=_("cmd-status-clear-name"), description=_("cmd-status-clear-description"))
async def status_clear_command(self: Utility, ctx: commands.Context[Bot]) -> None:
    ...
    await self.reply(ctx, _("msg-status-clear-status"), ephemeral=True)
